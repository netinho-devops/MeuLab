...
#			scp apsdlv@indlin574:/APS/delivery/dlvoms/v900/SP${SP}/PB${PB}/HF${HF}/XDK/*.jar .
			scp apsdlv@illin1132:/APS/delivery/dlvoms/v900/SP${SP}/PB${PB}/HF${HF}/RT/*.jar .
			scp apsdlv@illin1132:/APS/delivery/dlvoms/v900/SP${SP}/PB${PB}/HF${HF}/XDK/*.jar .
			chmod 755 *
...
copyAMSSPackages()
{
	...
	set -x
		COUNT_CLIENT=$(ls -1 UXFramework1-general-9.3.0.0.*.jar | wc -l)

		if [ ${COUNT_CLIENT} -eq 0 ]; then
			echo "UXFramework1-general jar file was not found!"
			exit 9
		fi

		if [ ${COUNT_CLIENT} -gt 1 ]; then
			LATEST_CLIENT=$(ls -1 UXFramework1-general-9.3.0.0.*.jar | sort | tail -n 1)
			echo "Removing duplicated jars"
			ls UXFramework1-general* | grep -v "${LATEST_CLIENT}" | xargs rm -vf
		fi
	set +x
