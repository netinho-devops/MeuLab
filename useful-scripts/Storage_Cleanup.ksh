...
# -------------------------------------------------------------------------------------------------
print "\nStep #3 - Find all storages (${PRODUCT}) which are at least $DAYS_OLD_TO_DEL days old"
print "========================================================="
...
do
   ...
   DIFF_DATE=`echo "set head off feed off pagesize 0
              SELECT (SYSDATE - TO_DATE('$CREATION_DATE')) 
              FROM    DUAL; "                                  | sqlplus -s $TOOLS_DB` 

   if [ ${DIFF_DATE} -gt ${DAYS_OLD_TO_DEL} ]
   then           
         ALL_STORAGE=${ALL_STORAGE}" "${GEN_STORAGE}
         printf "%-30s%-30s\n" ${GEN_STORAGE} ${CREATION_DATE}
   fi 
done


# -------------------------------------------------------------------------------------------------
print "\nStep #4 - Find all storages (${PRODUCT}) which are at least ${DAYS_OLD_TO_DEL} days old and not in use"
print "=========================================================================="
...
do
    if [ $(grep -c ${STRG} ${FILE_USED_STORAGE}) -eq 0 ];  then   
        STG_LOCATION=`ls -1d /XPISTORAGE/*/*/ST_* | grep ${STRG}`
		...
		if [[ $(echo ${STG_VERSION}| tr "[:upper:]" "[:lower:]") -eq "3000" ]]; then
			if [[ "${PRODUCT}" == "SLR"* ]]; then
				...
				STG_RELEASE=$(cd /XPISTORAGE/BUILD_RELEASE/${PRODUCT}/v${STG_VERSION}/*/BN_${STG_BUILD}; pwd)
			fi
			...
			if [ "${IS_REPORT}" == "Y" ]; then
				...
				GENESIS_HOME="/vivnas/viv/vivtools/GENESIS/3000"
				...
				#sqlplus -s $TOOLS_DB << EOF
				#	delete GNS_STORAGE_REGISTRATION where STORAGE_NAME='$STRG';
		 		#	exit
			 	#EOF	
			fi
			DEL_NUM=$((DEL_NUM+1))
