...
########	files	########
LOG_FILE="${HOME}/scripts/AutoServersDiagram/Logs/AutoServersDiagram.log"
HTML_FILE="${HOME}/scripts/AutoServersDiagram/HTML/AutoServersDiagram.html"
#Place to change
MANUAL_RUNNING_DEFINITION="${HOME}/scripts/AutoServersDiagram/Definition/VIVO_SERVERS_DEFINITIONS_FOR_MANUALLY_RUNNING.txt"
AUTOMATIC_RUNNING_DEFINITION="${HOME}/scripts/AutoServersDiagram/Definition/VIVO_SERVERS_DEFINITION_FOR_AUTONOMOUS_DIAGRAM.txt"

PROJECT_DEFINITION="${HOME}/scripts/AutoServersDiagram/Definition/PROJUCT_DETAILS.txt"
TEMP_FILE=${HOME}/scripts/AutoServersDiagram/tmp/TEMP_FILE
TEMP_ROW=${HOME}/scripts/AutoServersDiagram/tmp/TEMP_ROW
INT_ARRAY=${HOME}/scripts/AutoServersDiagram/tmp/INT_SERVERS_ARRAY
DB_ARRAY=${HOME}/scripts/AutoServersDiagram/tmp/DB_SERVERS_ARRAY
CC_ARRAY=${HOME}/scripts/AutoServersDiagram/tmp/CC_SERVERS_ARRAY
DB_INSTANCES_DIR=${HOME}/scripts/AutoServersDiagram/tmp/DBInstances
PRODUCTS_DIR=${HOME}/scripts/AutoServersDiagram/tmp/Products

#constants
TOOL_SERVER_COL_FOR_ALIGNEMENT_TABLE=3
TOOL_SERVER_COL_FOR_PRODUCTS_HEIGHT_TABLE=5
INSTANCES_NUMBERS_FOR_TABLE_HEIGHT=10
########	Global variables	########
_projectName=""
_projectDL=""
_projectEnsightDB=""
...
function generateHTMLFile
{
  ...
  while [ ${i} -lt ${INSTANCES_NUMBERS_FOR_TABLE_HEIGHT} ]
  #while [ ${i} -lt $((_DBIndex)) ]
  do
    echo "<tr>" >> ${HTML_FILE}
  ...
  #Tool servers
  echo "<ul>" >> ${HTML_FILE}
  echo "<li><font face="Bradley Hand ITC" size="4"><i><u>Infra Server Tool</u></i></font></li>"  >> ${HTML_FILE}
  echo "</ul>" >> ${HTML_FILE}
  ...
  i=1
  debugMode "_intIndex" "${_intIndex}"
  while [ ${i} -lt ${TOOL_SERVER_COL_FOR_ALIGNEMENT_TABLE} ]
    do
      if [ ${i} -gt 1 ]
        then
  ...
  echo "</tr>" >> ${HTML_FILE}
    ...
    j=1
    while [ ${j} -lt ${TOOL_SERVER_COL_FOR_PRODUCTS_HEIGHT_TABLE} ]
    do
      i=1
      echo "<tr>" >> ${HTML_FILE}
      while [ ${i} -lt ${TOOL_SERVER_COL_FOR_ALIGNEMENT_TABLE} ]
...
#          then
          ...
          do
              if [ "${i}" -lt "${_ccIndex}" ]
                then
                  echo "<td width=\"${WIDTH_COL}\"><font face=\"${FONT_SERVER_FACE}\" size=\"${FONT_SERVER_SIZE}\"><center>"${_ccArray[${i}]}"</center></font></td>" >> ${HTML_FILE}
              else
                  echo "<td width=\"${WIDTH_COL}\"><font face=\"${FONT_SERVER_FACE}\" size=\"${FONT_SERVER_SIZE}\"><center></center></font></td>" >> ${HTML_FILE}
              fi
              i=$((i+1))
...
From:${FROM_MAIL}
To:${TO_MAIL}
Subject: ${_projectName} DVC Servers
